...
RUN apt-get update && \
    ...
    python3 -m pip install --no-cache-dir setuptools numpy==1.18.0 && \
    python3 -m pip install --no-cache-dir rasterio==1.1.1 pyproj==2.4.0 requests geojson Pillow==7.2.0
